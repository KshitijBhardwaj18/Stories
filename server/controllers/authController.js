...
export const signup = async (req, res, next) => {
  ...
  try {
    await newUser.save();
    console.log("SignUp: from " + newUser.email);
    res.status(201).json({
...
export const signin = async (req, res, next) => {
  ...
  try {
    ...
    if (validUser.lockUntil && validUser.lockUntil > Date.now()) {
      return next(
        errorHandler(400, "Account is locked. Please try again later.")
      );
    }
    ...
    if (!isMatch) {
      ...
      if (validUser.loginAttempts >= process.env.MAX_LOGIN_ATTEMPTS) {
        validUser.lockUntil =
          Date.now() + process.env.OTP_LOCK_TIME * 60 * 1000;
      }
    ...
    console.log("Signin: from " + validUser.email);
  ...
  try {
    ...
    if (user.loginOtpAttempts >= process.env.MAX_LOGIN_ATTEMPTS) {
      if (user.loginOtpAttemptsExpire > Date.now()) {
        console.log("Too many OTP attempts. Please try again later.");
        return res
          .status(400)
          .json({ message: "Too many OTP attempts. Please try again later." });
      } else {
    ...
    if (Number(user.loginOtp) !== Number(otp)) {
      ...
      if (user.loginOtpAttempts >= process.env.MAX_LOGIN_ATTEMPTS) {
        user.loginOtpAttemptsExpire = new Date(
          Date.now() + process.env.OTP_LOCK_TIME * 60 * 1000
        ); // Lock for OTP_LOCK_TIME minutes
      }
